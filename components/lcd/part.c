...
static misc_device_entry_t misc_device_entry_list[] = {
...
static fpga_family_t fpga_family_list[] = {
...
static const size_t fpga_list_size = sizeof(fpga_list) / sizeof(fpga_entry_t);

const fpga_model_t *find_fpga_model(uint32_t id) {
    for (size_t i = 0; i < fpga_list_size; ++i) {
        if (fpga_list[i].id == id) {
            return &fpga_list[i].model;
        }
    }
    return NULL;
}
