...
//#define TEST_FAT_FILE 1 
#ifdef TEST_FAT_FILE
//const char* test_file_path = "/data/target/uncrustify.cfg";
const char* test_file_path = "/data/rustify.cfg"; ///data/test.txt
//const char* test_file_path = "/data/test.txt";
void delete_file_in_data()
{

    if (unlink(test_file_path) == 0) {
        ESP_LOGI(TAG, "File deleted: %s", test_file_path);
    } else {
        ESP_LOGE(TAG, "Failed to delete file: %s", test_file_path);
    }
}

void list_files_in_data()
{
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        if (stat(full_path, &st) == 0) {
            ...
            ESP_LOGW(TAG, "Failed to get file info for: %s", full_path);
        }
    }
...
static void write_text_file()
{
    ...
    // Open file for writing ("w" = write, truncates if exists)
    ...
    const char* message = "Hello from ESP32-S3 FAT filesystem!\n";
    fprintf(f, "%s", message);  // Write the string
    fclose(f);  // Don't forget to close!

    ESP_LOGI(TAG, "File written: %s", test_file_path);
}
...
esp_err_t storage_init_filesystem(void)
{
    ...
    if (dir) {
        ...
        ESP_LOGE(TAG, "Failes: IR* dir = opendir()");
    }
...
esp_err_t storage_write(const char *key, const char *value, size_t len)
{
    ...
    return ESP_FAIL;//aliDbg:TODO: below Not working, to be fixed
...
size_t storage_get_value_length(const char *key)
{
    ...
    return 0;//aliDbg:TODO: below Not working, to be fixed
