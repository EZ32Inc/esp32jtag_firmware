...
static esp_err_t network_init(void)
{
    ...
    if (!s_network->adapter->init) {
        ESP_LOGE(TAG, "adapter->init is NULL");
...
static esp_err_t network_connect(void)
{
    ...
    if (!s_network->adapter->connect) {
        ESP_LOGE(TAG, "adapter->connect is NULL");
        return ESP_ERR_INVALID_STATE;
    }
...
static esp_err_t __attribute__((unused)) network_disconnect(void)
{
    ...
    if (!s_network->adapter->disconnect) {
        ESP_LOGE(TAG, "adapter->disconnect is NULL");
        return ESP_ERR_INVALID_STATE;
    }
...
esp_err_t network_get_sta_credentials(char *ssid, char *pass)
{
    ...
    strncpy(ssid, adap_prv->ssid, WIFI_SSID_LEN - 1);
    ssid[WIFI_SSID_LEN - 1] = '\0';
    strncpy(pass, adap_prv->pass, WIFI_PASS_LEN - 1);
    pass[WIFI_PASS_LEN - 1] = '\0';
    return ESP_OK;
}
