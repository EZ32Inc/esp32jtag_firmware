...
/* WIFI_CONNECTED_BIT, WIFI_DISCONNECTED_BIT, WIFI_STARTED_BIT defined in network_mngr.h */
...
/* Non-static so network_mngr_ota.c can access them (declared in network_mngr.h) */
EventGroupHandle_t s_event_group = NULL;
esp_netif_t *s_sta_netif = NULL;
esp_netif_t *s_ap_netif = NULL;

// Declare event handlers as non-static if network_mngr_ota.c needs to unregister them
// Or ensure unregistration happens only in the scope where they are registered
...
static void wifi_event_handler(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
    ...
    case WIFI_EVENT_AP_STOP:
        ...
        // This bit could be set for `network_mngr_disconnect_wifi` if it waits for it
...
esp_err_t network_mngr_init_sta(const char *ssid, const char *pass)
{
    ...
    // Only create if not already created (e.g., from provisioning)
    if (!s_sta_netif) {
...
err:
    if (s_sta_netif) { // Only destroy if we created it in this function
...
esp_err_t network_mngr_connect_sta(unsigned int max_retry)
{
    for (size_t i = 0; i < max_retry; i++) {
        ...
        if(i==0){
            Paint_DrawString_EN(X_OFFSET, 10, "ST Mode, Connecting...", &CURR_FONT, BLACK, MAGENTA);
        }

        // Clear connection bits before attempting to connect
        xEventGroupClearBits(s_event_group, WIFI_CONNECTED_BIT | WIFI_DISCONNECTED_BIT);
        ...
        // Wait for connection/disconnection event
        EventBits_t bits = xEventGroupWaitBits(s_event_group,
                                               WIFI_CONNECTED_BIT | WIFI_DISCONNECTED_BIT,
                                               pdTRUE,
                                               pdFALSE,
                                               pdMS_TO_TICKS(10000)); // Wait up to 10 seconds for connect/disconnect

        if (bits & WIFI_CONNECTED_BIT) {
            ESP_LOGI(TAG, "wifi sta is connected");
            Paint_DrawString_EN(X_OFFSET, 10, "ST Mode, Connected!   ", &CURR_FONT, BLACK, GREEN);
            return ESP_OK;
    ...
    Paint_DrawString_EN(X_OFFSET, 10, "ST Mode, Failed!      ", &CURR_FONT, BLACK, GREEN);
    return ESP_FAIL;
}
...
esp_err_t network_mngr_init_prov(const char *ssid, const char *pass, const void *http_handle)
{
    ...
    if (!s_ap_netif) { // Only create if not already created
        s_ap_netif = esp_netif_create_default_wifi_ap();
    }
    if (!s_sta_netif) { // Only create if not already created
        s_sta_netif = esp_netif_create_default_wifi_sta();
    }
...
static esp_err_t network_mngr_get_ip(esp_netif_t *netif, char **ip)
{
    ...
    char ip_addr[16] = {0};
    // Check if IP is assigned (e.g., not 0.0.0.0)
    if (ip_info.ip.addr == 0) {
        ESP_LOGW(TAG, "IP address not assigned yet for netif.");
        *ip = strdup("0.0.0.0"); // Or return error
        return ESP_FAIL;
    }
